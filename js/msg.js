...
(() => {
  ...
  function getExtBg() {
    const fn = chrome.extension && chrome.extension.getBackgroundPage;
    const bg = fn && fn();
  ...
  const apiHandler = !msg.isBg && {
    ...
    async apply({path}, thisObj, args) {
      const bg = getExtBg() ||
        chrome.tabs && browser.runtime.getBackgroundPage &&
          await browser.runtime.getBackgroundPage().catch(() => {});
