  ...
  function doInject(tabId, frameId, script) {
    ...
    msg.sendTab(tabId, {method: 'ping'}, options)
      ...
      .then(pong => {
        ...
        executeScript(tabId, Object.assign({files: script.js}, options), ignoreChromeError);
      });
  }

  function executeScript(tabId, {files, frameId, allFrames, matchAboutBlank}, cb) {
    if (chrome.scripting) {
      chrome.scripting.executeScript({
        target: Object.assign(
          {tabId},
          frameId != null ? {frameIds: [frameId]} : {allFrames}
        ),
      ...
      for (const file of files) {
        chrome.tabs.executeScript(tabId, {file, frameId, allFrames, matchAboutBlank}, cb);
      }
    }
  }
