...
bgReady.all.then(() => {
  ...
  chrome.webNavigation.onCommitted.addListener(({tabId}) => {
    executeScript(tabId, '/content/install-hook-greasyfork.js');
  }, {
    url: [
   ...
   */
  chrome.webNavigation.onCommitted.addListener(({tabId}) => {
    executeScript(tabId, '/content/install-hook-userstylesworld.js');
  }, {
    url: [
    ...
    chrome.webNavigation.onDOMContentLoaded.addListener(async ({tabId, frameId}) => {
      if (frameId &&
        ...
        for (const file of chrome.runtime.getManifest().content_scripts[0].js) {
          executeScript(tabId, file, frameId);
        }
      }
    }, {
  ...
  function executeScript(tabId, file, frameId) {
    if (chrome.scripting) {
