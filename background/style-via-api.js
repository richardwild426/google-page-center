...
/**
 * Uses chrome.scripting.insertCSS in MV3 and chrome.tabs.insertCSS in MV2.
 */
...
(() => {
  ...
  function styleApply({id = null, ignoreUrlCheck = false}, {tab, frameId, url}) {
    ...
    return API.styles.getSectionsByUrl(url, id).then(sections => {
      ...
      for (const section of Object.values(sections)) {
        ...
        tasks.push(insertCSS(tab.id, frameId, code));
      }
  ...
  function removeCSS(tabId, frameId, code) {
    ...
    return browser.tabs.removeCSS(tabId, {frameId, code, matchAboutBlank: true}).catch(onError);
  }

  function insertCSS(tabId, frameId, code) {
    if (chrome.scripting) {
